...
server.on("connection", async (socket, req) => {
    socket.on("message", message => {
        ...
        switch (_message.type) {
            ...
            case "TYPE_REQUEST_CHECK": opened.filter(node => node.address === _message.data)[0].socket.send(JSON.stringify(produceMessage("TYPE_SEND_CHECK", JSON.stringify([File_BlockChain.getLastBlock(), File_BlockChain.transactions, File_BlockChain.difficulty]))));
                break;
            ...
            case "TYPE_REQUEST_INFO":
                opened.filter(node => node.address === _message.data)[0].socket.send(JSON.stringify(produceMessage(
                    "TYPE_SEND_INFO",
                    [File_BlockChain.difficulty, File_BlockChain.transactions]
                )));

                break;

            case "TYPE_SEND_INFO":
                [File_BlockChain.difficulty, File_BlockChain.transactions] = _message.data;
